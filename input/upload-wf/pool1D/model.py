...
class Model(nn.Module):
    def __init__(self, weights=None, input_size=512, num_classes=17, vocab_size=64, pool_win_frms=15, input_FpS=2, pool="NetVLAD"):
        """
        INPUT: a Tensor of shape ( batch_size, pool_win_frms, feature_size )
        OUTPUTS: a Tensor of shape (batch_size,num_classes+1)
        """

        super(Model, self).__init__()

        self.pool_win_frms = pool_win_frms
        ...
        if self.pool == "MAX":
            self.pool_layer = nn.MaxPool1d(self.pool_win_frms, stride=1)
            self.fc = nn.Linear(input_size, self.num_classes+1)

        if self.pool == "MAX++":
            self.pool_layer_before = nn.MaxPool1d(int(self.pool_win_frms/2), stride=1)
            self.pool_layer_after = nn.MaxPool1d(int(self.pool_win_frms/2), stride=1)
            self.fc = nn.Linear(2*input_size, self.num_classes+1)


        if self.pool == "AVG":
            self.pool_layer = nn.AvgPool1d(self.pool_win_frms, stride=1)
            self.fc = nn.Linear(input_size, self.num_classes+1)

        if self.pool == "AVG++":
            self.pool_layer_before = nn.AvgPool1d(int(self.pool_win_frms/2), stride=1)
            self.pool_layer_after = nn.AvgPool1d(int(self.pool_win_frms/2), stride=1)
            self.fc = nn.Linear(2*input_size, self.num_classes+1)
